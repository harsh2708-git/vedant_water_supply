...
import com.production.vedantwatersupply.utils.CommonUtilsJava;
...
public class NumericEditText extends CustomEditText {
    private final char GROUPING_SEPARATOR = DecimalFormatSymbols.getInstance(Locale.ENGLISH).getGroupingSeparator();
    private final char DECIMAL_SEPARATOR = DecimalFormatSymbols.getInstance(Locale.ENGLISH).getDecimalSeparator();
    private final String LEADING_ZERO_FILTER_REGEX = "^0+(?!$)";

    private String mDefaultText = null;
    private String mPreviousText = "";
    private String mNumberFilterRegex = String.format("[^\\d\\%s]", DECIMAL_SEPARATOR);
    ...
    private final TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (CommonUtilsJava.getNumeric(s.toString()) > maxValue) {
                validateLock = true;
     ...
     *
     * @return numeric value or { Double.NaN} if not a number
     */
    public double getNumericValue() {
        ...
        try {
            return NumberFormat.getInstance(Locale.ENGLISH).parse(original).doubleValue();
    ...
    public Editable getText() {
        return Editable.Factory.getInstance().newEditable(String.valueOf(getNumericValue()));
    }
    ...
    private String format(final String original) {
        final String[] parts = original.split(String.format("\\%s", mDecimalSeparator), -1);
